...
public class UtilitatsData {

    private UtilitatsData() {}

    public static java.util.Optional<Integer> analitzarYear(String date) {
    ...
    public static java.time.LocalDate analitzarIsoDate(String date) {
        if (date == null || date.isBlank()) return null;
        try { return java.time.LocalDate.parse(date.trim()); }
        catch (java.time.format.DateTimeParseException e) { return null; }
    }

    public static String formatejarDateForDisplay(String isoDate) {
        if (isoDate == null || isoDate.isBlank()) return "";
        java.time.LocalDate d = analitzarIsoDate(isoDate);
