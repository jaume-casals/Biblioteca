...
public final class ParseHelpers {
    ...
    private static <T> T orNull(String s, java.util.function.Function<String, T> p) {
        if (s == null) return null;
        String t = s.trim();
        if (t.isEmpty()) return null;
        try { return p.apply(t); }
        catch (NumberFormatException e) { return null; }
    }

    public static Long analitzarLongOrNull(String s) { return orNull(s, Long::parseLong); }

    public static Integer analitzarIntOrNull(String s) { return orNull(s, Integer::parseInt); }

    public static Double analitzarDoubleOrNull(String s) { return orNull(s, Double::parseDouble); }

    private static <T> T parseStrict(String s, T def, String errKey, String fieldKey,
                                     List<String> errors, java.util.function.Function<String, T> p) {
        if (s == null) return def;
        String t = s.trim();
        if (t.isEmpty()) return def;
        try { return p.apply(t); }
        catch (NumberFormatException e) {
            errors.add(I18n.t(errKey, I18n.t(fieldKey)));
            return def;
        }
    }

    public static long parseLong(String s, long defaultValue, String fieldKey, List<String> errors) {
        return parseStrict(s, defaultValue, "val_must_be_int", fieldKey, errors, Long::parseLong);
    }

    public static int parseInt(String s, int defaultValue, String fieldKey, List<String> errors) {
        return parseStrict(s, defaultValue, "val_must_be_int", fieldKey, errors, Integer::parseInt);
    }

    public static double parseDouble(String s, double defaultValue, String fieldKey, List<String> errors) {
        return parseStrict(s, defaultValue, "val_must_be_number", fieldKey, errors, Double::parseDouble);
    }

    /** Converteix una cadena buida o només espais en {@code null}. */
    public static String blankToNull(String s) {
        if (s == null || s.isEmpty()) return null;
        return s;
    }

    /** Parteix una llista d'autors separats per comes en noms individuals trimats. */
    public static List<String> splitAutors(String text) {
