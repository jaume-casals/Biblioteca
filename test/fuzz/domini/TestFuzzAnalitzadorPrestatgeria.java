...
import domini.AnalitzadorPrestatgeria;
 ...
 * </ol>
 */
class TestFuzzAnalitzadorPrestatgeria {

    @FuzzTest(maxDuration = "30s")
    void fuzz(FuzzedDataProvider data) {
        String raw = data.consumeRemainingAsString();

        List<AnalitzadorPrestatgeria.ShelfEntry> entries = AnalitzadorPrestatgeria.analitzarShelfEntries(raw);
        assertThat(entries)
        ...
        for (int i = 0; i < entries.size(); i++) {
            AnalitzadorPrestatgeria.ShelfEntry e = entries.get(i);
            assertThat(e.nom())
        ...
        String rejoined = AnalitzadorPrestatgeria.joinShelfEntries(entries);
        List<AnalitzadorPrestatgeria.ShelfEntry> reparsed = AnalitzadorPrestatgeria.analitzarShelfEntries(rejoined);
