...
public class ServerConect {
	...
	public void afegirLlibre(Llibre llibre) {
		String query = 
		" INSERT INTO llibre (`ISBN`, `nom`, `autor`, `any`, `descripcio`, `valoracio`, `preu`, `llegit`, `imatge`) VALUES"
				+ " (?, ?, ?, ? ,? ,? ,? ,?, ? )";
		if (llibre != null) {
			try {
				...
				if (llibre.getAutor() != null) preparedStmt.setString(3, llibre.getAutor());
				else preparedStmt.setString(3, "");

				if (llibre.getAny() != null) preparedStmt.setInt(4, llibre.getAny());
				else preparedStmt.setInt(4, 0);

				if (llibre.getDescripcio() != null) preparedStmt.setString(5, llibre.getDescripcio());
				else preparedStmt.setString(5, "");

				if (llibre.getValoracio() != null) preparedStmt.setDouble(6, llibre.getValoracio());
				else preparedStmt.setDouble(6, 0.0);

				if (llibre.getPreu() != null) preparedStmt.setDouble(7, llibre.getPreu());
				else preparedStmt.setDouble(7, 0.0);

				if (llibre.getLlegit() != null) preparedStmt.setBoolean(8, llibre.getLlegit());
				else preparedStmt.setBoolean(8, false);

				if (llibre.getPortada() != null) preparedStmt.setString(9, llibre.getPortada());
				else preparedStmt.setString(9, "portades/default_cover.png");
