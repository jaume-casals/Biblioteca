...
8. **Patch local tests** (see § Phase 7 — `test/` is gitignored).
9. **`make compile && make test`**
10. **Manual smoke:** `make run` — add/edit book, import/export, config dialog; confirm nothing listens on port 7070.

---

## Phase 7 — Local test suite (`test/`)

`test/BibliotecaTest.java` and `test/BibliotecaJUnit5Test.java` are **not in git** (see `.gitignore`). After removing `src/api/`, web UI, and mode picker code, those files still reference deleted types unless you update them.

**Automated patch (recommended on each machine that runs `make test`):**
...
- Rewrites `interficie.EnActualizarBBDD` → `presentacio.listener.EnActualizarBBDD` (interface moved under `presentacio.listener`).
- **Removes** any test method whose body still mentions `api.*`, `ApiServer`, `HttpRouter`, `JsonMapper`, `ModeSelectorDialog`, `Ejecutable.resolveMode`, `Config.setLastMode` / `getLastMode`, `LibraryGraph`, etc.
- Drops matching `testFoo();` calls from `BibliotecaTest.main` when those methods were removed.

**Optional replacements** (not generated by the script):

| Old coverage | Swing-only alternative |
|--------------|------------------------|
| `api.JsonMapper` round-trip | `BookExporter.exportJSON` + `JsonImporter.run` on a temp file |
| `Ejecutable.resolveMode` / `Config.lastMode` | Assert `main(new String[]{"--web"})` exits 1 (subprocess) or document manual check |
| `api.ApiServer` HTTP smoke | Delete — use Swing import/export and `checkBiblio/UIAudit` instead |

**`checkBiblio/run.sh`:** classpath no longer requires `javalin` / `kotlin-stdlib` JARs.
...
[ ] python scripts/patch_tests_after_web_removal.py  (local test/ only)
[ ] make compile && make test
