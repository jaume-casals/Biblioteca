...
JAVA_PATH = os.path.join(REPO, "src", "herramienta", "i18n", "I18n.java")
PROP_PATHS = {
...
_ESCAPE_CHARS = set('ntr"\\\'0')

def _escape(s, quote_escape='"'):
    out = []
    i = 0
    n = len(s)
    while i < n:
        c = s[i]
        if c == '\\':
            if i + 1 < n and s[i + 1] in _ESCAPE_CHARS:
                out.append('\\' + s[i + 1])
                i += 2
            else:
                out.append('\\\\')
