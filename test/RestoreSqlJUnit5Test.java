...
class RestoreSqlJUnit5Test {
    ...
    void dashDashInsideStringLiteralSurvivesRestore() throws Exception {
        ...
        Llibre book = LlibreValidator.checkLlibre(isbn, "Pre-war ed.", null, 1939, null, null, null, null, null);
        book.setNotes("pre--war ed.; a note; another note");
        cd.addLlibre(book);

        File backup = File.createTempFile("sql_dash_inside_quote_", ".sql");
        backup.deleteOnExit();
        cd.backupToSQL(backup);

        ControladorDomini.resetForTest();
        ControladorPersistencia.resetForTest();
        ControladorDomini cd2 = ControladorDomini.getInstance();
        cd2.restoreFromSQL(backup);

        assertThat(cd2.getLlibre(isbn).getNotes())
            .as("Notes with '--' inside a string literal must survive restore")
            .isEqualTo("pre--war ed.; a note; another note");
        assertThat(cd2.getLlibre(isbn).getNom()).isEqualTo("Pre-war ed.");
    }
    ...
    void lineCommentIsStillStripped() throws Exception {
        ControladorDomini cd = ControladorDomini.getInstance();
        long isbn = 9780306406157L;
        Llibre book = LlibreValidator.checkLlibre(isbn, "Comment test", null, 2020, null, null, null, null, null);
        cd.addLlibre(book);
        ...
        for (Llibre l : cp.getAllLlibres()) dbIsbns.add(l.getISBN());
        assertThat(dbIsbns).contains(9781111111111L, 9782222222222L, isbn);
        assertThat(cd.getLlibre(isbn).getNom()).isEqualTo("Comment test");
    }

    @Test
    @DisplayName("A3: DELETE FROM and UPDATE … SET on a tampered file are silently ignored")
    void deleteAndUpdateAreRejectedByAllowlist() throws Exception {
        ControladorDomini cd = ControladorDomini.getInstance();
        long isbn = 9780306406157L;
        cd.addLlibre(LlibreValidator.checkLlibre(isbn, "Survivor", null, 2020, null, null, null, null, null));
        ...
        assertThat(cd.getLlibre(isbn).getNom()).isEqualTo("Survivor");
    }
}
