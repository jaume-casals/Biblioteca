...
class StressDominiTest {
    ...
    void bulkInsert1k() throws Exception {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            cd.addLlibre(new Llibre(isbn, "Book " + i, "Author " + i, 1900 + (i % 130),
                "desc", (double) (i % 11), 0.0, i % 2 == 0, ""));
        }
        assertThat(cd.getSize()).isEqualTo(1000);
        // Spot-check the first and last
        assertThat(cd.getLlibre(9780306400000L).getNom()).isEqualTo("Book 0");
        assertThat(cd.getLlibre(9780306400999L).getNom()).isEqualTo("Book 999");
    }
    ...
    void bulkInsert5k() throws Exception {
        ...
        for (int i = 0; i < 5000; i++) {
            long isbn = 9780306400000L + i;
            cd.addLlibre(new Llibre(isbn, "B" + i, null, null, null, null, null, false, null));
        }
            ...
            workers.add(new Thread(() -> {
                try {
                    ...
                    for (int i = 0; i < perThread; i++) {
                        ...
                        cd.addLlibre(new Llibre(isbn, "T" + tid + "B" + i, null, null, null, null, null, false, null));
                        // occasionally read
                        if (i % 5 == 0) cd.getAllLlibres().size();
                    }
                } catch (Exception e) {
    ...
    void bulkTag() throws Exception {
        ...
        for (int i = 0; i < 1000; i++) {
            long isbn = 9780306400000L + i;
            cd.addLlibre(new Llibre(isbn, "B" + i, null, null, null, null, null, false, null));
            cd.addLlibreToTag(isbn, (i % 5) + 1);
        }
        // Each tag should have exactly 200 books
        for (int i = 0; i < 5; i++) {
            LlibreFilter f = LlibreFilterBuilder.of().tagId(i + 1).build();
    ...
    void bulkShelf() throws Exception {
        ControladorDomini cd = ControladorDomini.getInstance();
        Llista shelf = cd.addLlista("Bulk");
        for (int i = 0; i < 1000; i++) {
            long isbn = 9780306400000L + i;
            cd.addLlibre(new Llibre(isbn, "B" + i, null, null, null, 0.0, 0.0, false, null));
            cd.addLlibreToLlista(isbn, shelf.getId(), 0.0, false);
        }
        assertThat(cd.getCountInLlista(shelf.getId())).isEqualTo(1000);
        assertThat(cd.getLlibresInLlista(shelf.getId())).hasSize(1000);
    }
    ...
    void bulkBackupRestore() throws Exception {
        ...
        for (int i = 0; i < 500; i++) {
            long isbn = 9780306400000L + i;
            cd.addLlibre(new Llibre(isbn, "Bulk " + i, "Author", 2000, "", 5.0, 0.0, false, ""));
        }
        ...
        cd2.restoreFromSQL(tmp);

        assertThat(cd2.getSize()).isEqualTo(500);
        assertThat(cd2.getLlibre(9780306400000L).getNom()).isEqualTo("Bulk 0");
        assertThat(cd2.getLlibre(9780306400499L).getNom()).isEqualTo("Bulk 499");
    }
    ...
    void paginationClampAtScale() throws Exception {
        ...
        for (int i = 0; i < 250; i++) {
            long isbn = 9780306400000L + i;
            cd.addLlibre(new Llibre(isbn, "P" + i, null, null, null, null, null, null, null));
        }
