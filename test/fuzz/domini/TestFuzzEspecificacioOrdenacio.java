 ...
 * </ol>
 */
class TestFuzzEspecificacioOrdenacio {

    private static final List<String> KNOWN_COLUMNS = List.of(
        EspecificacioOrdenacio.COL_ISBN, EspecificacioOrdenacio.COL_NOM, EspecificacioOrdenacio.COL_ANY,
        EspecificacioOrdenacio.COL_VALORACIO, EspecificacioOrdenacio.COL_PREU);
    ...
    void fuzz(FuzzedDataProvider data) {
        ...
        EspecificacioOrdenacio s = new EspecificacioOrdenacio(col, asc);
        ...
        Comparator<Llibre> cmp = EspecificacioOrdenacio.comparator(col);
        ...
        Comparator<Llibre> cmp2 = EspecificacioOrdenacio.comparator(s.column());
        assertThat(cmp2).isNotNull();

        // Registry sanity: no duplicate column names.
        Set<String> seen = new HashSet<>();
        for (String k : EspecificacioOrdenacio.columns().keySet()) {
            assertThat(seen.add(k)).as("duplicate column key in registry: %s", k).isTrue();
        }
        // Registry must at least cover the 5 known columns.
        assertThat(EspecificacioOrdenacio.columns().keySet()).containsAll(KNOWN_COLUMNS);
    }
}
