...
public class ServeiCopiaSeguretat {
    ...
    public void copiaSegToSQL(File file, List<Llibre> bib, List<Llista> llistes, List<Tag> tags) throws java.io.IOException, java.sql.SQLException {
        ...
        try (java.io.PrintWriter pw = new java.io.PrintWriter(
            ...
            for (persistencia.AutorRow row : cp.obtenirAllAutorRows()) {
                escriureAutorINSERT(pw, row.id(), row.nom());
            }
            for (persistencia.LlibreAutorRow row : cp.obtenirAllLlibreAutorRows()) {
                escriureLlibreAutorINSERT(pw, row.isbn(), row.autorId());
            }
            for (Llista ll : llistes) {
                escriureLlistaINSERT(pw, ll);
            }
            for (persistencia.LlibreLlistaRow row : cp.obtenirAllLlibreLlista()) {
                escriureLlibreLlistaINSERT(pw, row);
            }
            for (Tag t : tags) {
                escriureTagINSERT(pw, t);
            }
            for (persistencia.LlibreTagRow row : cp.obtenirAllLlibreTag()) {
                escriureLlibreTagINSERT(pw, row);
            }
            for (persistencia.PrestecRow row : cp.obtenirAllPrestecs()) {
                escriurePrestecINSERT(pw, row);
            }
            for (persistencia.LecturaRow row : cp.obtenirAllLectures()) {
    ...
    private void escriureLlibreLlistaINSERT(java.io.PrintWriter pw, persistencia.LlibreLlistaRow row) {
    ...
    private void escriureLlibreTagINSERT(java.io.PrintWriter pw, persistencia.LlibreTagRow row) {
    ...
    private void escriurePrestecINSERT(java.io.PrintWriter pw, persistencia.PrestecRow row) {
    ...
    private void escriureLecturaINSERT(java.io.PrintWriter pw, persistencia.LecturaRow row) {
