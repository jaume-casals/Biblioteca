 ...
 * container.
 */
class LlibreInvariantPropertiesTest {
    ...
    @Property(tries = 20)
    void addLlibre_thenDelete_sizeUnchanged_isbnGone(
    ...
    void prestarLlibre_thenRetornar_loanCleared(
        ...
        assertThat(cd.getLoanedISBNs()).contains(isbn);

        cd.retornarLlibre(isbn);
        assertThat(cd.getLoanedISBNs()).doesNotContain(isbn);
    }
    ...
    void shelf_addThenRemove_isbnGoneFromShelf(
        ...
        cd.addLlibre(Llibre.builder()
            ...
            .descripcio("").valoracio(0.0).preu(0.0).llegit(false).imatge("")
            .build());
        Llista shelf = cd.addLlista("Shelf " + shelfName);
        int llistaId = shelf.getId();

        cd.addLlibreToLlista(isbn, llistaId, 0.0, false);
        assertThat(cd.getLlibresInLlista(llistaId)).extracting(Llibre::getISBN).contains(isbn);

        cd.removeLlibreFromLlista(isbn, llistaId);
        assertThat(cd.getLlibresInLlista(llistaId)).extracting(Llibre::getISBN).doesNotContain(isbn);
    }
    ...
    void tag_addThenRemove_isbnGoneFromTag(
        ...
        cd.addLlibre(Llibre.builder()
            ...
            .descripcio("").valoracio(0.0).preu(0.0).llegit(false).imatge("")
            .build());
        int tagId = cd.addTag("Tag " + tagName).getId();

        cd.addLlibreToTag(isbn, tagId);
        assertThat(cd.getLlibresWithTag(tagId)).contains(isbn);

        cd.removeLlibreFromTag(isbn, tagId);
        assertThat(cd.getLlibresWithTag(tagId)).doesNotContain(isbn);
    }
     ...
     */
    @Property(tries = 20)
    void filterAuthor_isCaseInsensitive(@ForAll long seed) {
        ...
        LlibreFilter lower = LlibreFilterBuilder.of().autor("smith").build();
        LlibreFilter upper = LlibreFilterBuilder.of().autor("SMITH").build();
